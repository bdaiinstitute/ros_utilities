...
def test_static_tf_burst(ros: ROSAwareScope) -> None:
    ...
    tf_broadcaster.sendTransform(
        [world_to_fiducial_a_transform, footprint_to_body_transform]
    )

    tf_listener = TFListenerWrapper(ros.node)
    transform = tf_listener.lookup_a_tform_b(
        "footprint", "camera", timeout_sec=2.0, wait_for_frames=True
    )
    assert transform.transform.rotation.w == 1.0
    transform = tf_listener.lookup_a_tform_b(
        "world", "fiducial_a", timeout_sec=2.0, wait_for_frames=True
    )
