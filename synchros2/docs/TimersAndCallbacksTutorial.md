...
1. As part of the Getting Started Guide, you should have created a `synchros2_tutorials` packages.  We’ll add this file to it in `<workspace>/src/synchros2_tutorials/synchros2_tutorials/msg_processor_cb.py` and add the following code to it. 
    ...
    def main() -> None:
        mp = MsgProcessor()
        while rclpy.ok():
            ros_scope.ensure_node().get_logger().info(f"Processed message is {mp.processed_msg}")
            time.sleep(1.0)
...
Inside the main we create our message processor and then constantly print out the class variable.  This shows us that the callback is successfully running and processing while the main thread keeps going:
...
## I Thought We Weren’t Supposed to Use Timed Sleeps?

In general, it’s better to use ROS2 timers than timed sleeps largely because this respects “ROS time”, which is important for simulation.  In this case, our major use of timed sleeps is just to simulate doing some “work” - users should replace these with actual processing.  However we do use a loop+sleep pattern in main:
...
This isn’t actually a good pattern to follow.  We did it here because it’s simple to interpret and the printing is important to understanding what is happening.  However, this would be technically better to implement as a timer callback:
...
def main() -> None:
    ...
    timer = node.create_timer(timer_period_sec=1.0, callback=_print_callback)
